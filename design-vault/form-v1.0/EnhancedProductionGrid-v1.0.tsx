...
export const EnhancedProductionGrid: React.FC<EnhancedProductionGridProps> = ({
  ...
  const handleGenerateProposal = async () => {
    ...
    try {
      ...
      const proposalNumber = projectData.proposalNumber || `AB-2025-${Math.floor(Math.random() * 900000) + 100000}`;
                    ...
                    <div className={isMobile ? 'space-y-3' : 'space-y-6'}>
                      ...
                      <div className={`grid grid-cols-2 ${isMobile ? 'gap-2 p-2' : 'gap-4 p-4'} rounded-lg bg-anchor-blue-light border border-gray-200`}>
                        <div>
                          ...
                          <input
                            type='text'
                            value={projectData.proposalNumber || `AB-${Date.now().toString().slice(-6)}`}
                            onChange={e => updateProjectData({ proposalNumber: e.target.value })}
                            className={`${isMobile ? 'h-6 px-2 py-0.5 text-[10px]' : 'h-8 px-3 py-1 text-[11px]'} border border-gray-300 rounded text-gray-900 bg-white focus:border-anchor-blue focus:ring-1 focus:ring-anchor-blue focus:outline-none hover:border-gray-400 w-full`}
                            placeholder='AB-XXXX'
                          />
                        </div>
                        <div>
                          ...
                          <input
                            type='date'
                            value={projectData.proposalDate || new Date().toISOString().split('T')[0]}
                      ...
                      {/* Proposal Settings */}
                      <div className={`grid grid-cols-2 ${isMobile ? 'gap-2 p-2' : 'gap-4 p-4'} rounded-lg bg-gray-50 border border-gray-200`}>
                        <div>
                          <label className={`block ${isMobile ? 'text-[9px]' : 'text-[11px]'} font-medium text-gray-900 ${isMobile ? 'mb-0.5' : 'mb-1'}`}>Validity (Days)</label>
                          <input
                            type='number'
                        ...
                        <div>
                          <label className={`block ${isMobile ? 'text-[9px]' : 'text-[11px]'} font-medium text-gray-900 ${isMobile ? 'mb-0.5' : 'mb-1'}`}>Deposit Amount</label>
                          <input
                            type='number'

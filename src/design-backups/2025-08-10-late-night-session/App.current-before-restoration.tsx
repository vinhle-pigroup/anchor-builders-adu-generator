...
function FullPageDebugTool({ onBack }: { onBack: () => void }) {
  ...
  const [highlightMode, setHighlightMode] = useState(false); // Pure color highlighting mode
  ...
  }, [showNumbers, highlightMode, iframeLoaded, visibleSections, formFields]);
        ...
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center space-x-2'>
            ...
            >
              ...
              <span>{highlightMode ? 'Hide Highlights' : 'Show Highlights'}</span>
            </button>
...
function App() {
  ...
  const loadPreviousProposals = useCallback(() => {
    const proposals = getAllProposals();
    if (proposals.length === 0) {
      alert('No previous proposals found.');
      return;
    }
    ...
    const proposalList = proposals
      ...
      .map(
        p =>
          `${p.id} - ${p.client.firstName} ${p.client.lastName} (${new Date(p.lastModified || p.createdAt || '').toLocaleDateString()})`
      )
      .join('\n');

    const selectedProposal = prompt(
      `Choose a proposal to load:\n\n${proposalList}\n\nEnter the proposal ID (e.g., AB-1001):`
    );

    if (selectedProposal) {
      const proposal = proposals.find(p => p.id === selectedProposal.trim());
      if (proposal) {
        // Save current draft before switching
        if (formData.client.firstName || formData.client.lastName) {
          saveDraft(formData);
        }
        setFormData(proposal);
        setSuccessMessage(`Loaded proposal: ${proposal.id}`);
      } else {
        alert('Proposal not found. Please check the ID and try again.');
      }
    }
  }, [formData]);
  ...
  const createNewProposal = useCallback(() => {
    const hasData =
      formData.client.firstName || formData.client.lastName || formData.additionalNotes;

    if (hasData) {
      const confirmNew = window.confirm(
        'This will save the current proposal as a draft and create a new one. Continue?'
      );
    ...
    // Create new proposal with fresh ID
    const newProposal: AnchorProposalFormData = {
      id: generateNewProposalNumber(),
      client: {
        firstName: '',
        lastName: '',
    ...
    setSuccessMessage(`New proposal created: ${newProposal.id}`);
  }, [formData]);
  ...
  const generateTestData = useCallback(() => {
    ...
    setFormData({
      ...
      additionalNotes:
        'This is a sample proposal generated with test data for demonstration purposes.',
      timeline: '6-8 months',
      proposalDate: new Date().toLocaleDateString('en-US', {
        year: 'numeric',
        month: 'long',
      ...
      <>
        ...
        <EnhancedProductionGrid
          projectData={{
            ...
            propertyAddress:
              `${formData.client.address}, ${formData.client.city}, ${formData.client.state} ${formData.client.zipCode}`.trim(),
          ...
          onProjectDataUpdate={updates => {
            ...
            if (updates.propertyAddress) {
              const parts = updates.propertyAddress.split(', ');
              if (parts.length >= 3) {
                const [address, city, stateZip] = parts;
                const [state, zipCode] = stateZip.split(' ');
                updateClientData({ address, city, state, zipCode });
              }
            }
            if (updates.aduType) updateProjectData({ aduType: updates.aduType });
            if (updates.squareFootage !== undefined)
              setPricingData(prev => ({ ...prev, sqft: updates.squareFootage }));
            if (updates.bedrooms !== undefined)
              setPricingData(prev => ({ ...prev, bedrooms: updates.bedrooms }));
            if (updates.bathrooms !== undefined)
              setPricingData(prev => ({ ...prev, bathrooms: updates.bathrooms }));
            if (updates.hvacType) updateProjectData({ hvacType: updates.hvacType });
            if (updates.additionalNotes !== undefined)
              updateProjectData({ additionalNotes: updates.additionalNotes });
          }}
          onPricingDataUpdate={setPricingData}
        />
      </>
    );
  }

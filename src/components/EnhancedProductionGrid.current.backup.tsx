...
export const EnhancedProductionGrid: React.FC<EnhancedProductionGridProps> = ({
  ...
  return (
    <div className='flex flex-col h-screen bg-gradient-to-br from-stone-100 to-amber-50'>
      {/* Top Header with Anchor Builders Logo */}
      <div className='bg-white/95 backdrop-blur border-b border-amber-200/30 px-6 py-3 flex items-center justify-between shadow-sm'>
        {/* Left: Anchor Builders Logo and Title */}
        <div className='flex items-center gap-3'>
          <div className='w-10 h-10 bg-blue-600 rounded flex items-center justify-center'>
            <span className='text-white font-bold text-lg'>⚓</span>
          </div>
          <div>
            <h1 className='text-xl font-semibold text-gray-900'>Anchor Builders ADU Generator</h1>
            <p className='text-sm text-gray-500'>Internal Production Tool</p>
          </div>
        </div>
        
        {/* Right: Test Data Button and Dark Mode Toggle */}
        <div className='flex items-center gap-3'>
          <button 
            onClick={loadTestData}
            className='flex items-center gap-2 px-3 py-1.5 text-sm bg-orange-100 hover:bg-orange-200 border border-orange-300 rounded-md transition-colors font-medium'
          >
        ...
        <div className='bg-white border-l border-b p-4'>
          ...
          <div className='bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-lg p-4 mb-4'>
            <div className='text-center'>
              ...
              <div className='text-sm text-green-600 font-medium'>Total Project Cost</div>
            </div>
          </div>
          ...
          <div className='space-y-2 text-sm mb-4'>
            ...
            {pricingData.designServices > 0 && (
              <div className='flex justify-between'>
                ...
                <span className='font-medium text-green-600'>+${pricingData.designServices.toLocaleString()}</span>
              </div>
            )}
            {Object.values(pricingData.utilities || {}).reduce((sum, cost) => sum + cost, 0) > 0 && (
              <div className='flex justify-between'>
                <span className='text-gray-600'>Utilities:</span>
                <span className='font-medium text-green-600'>
                  +${Object.values(pricingData.utilities || {}).reduce((sum, cost) => sum + cost, 0).toLocaleString()}
                </span>
              </div>
            )}
            {Object.values(pricingData.additionalServices || {}).reduce((sum, cost) => sum + cost, 0) > 0 && (
              <div className='flex justify-between'>
                <span className='text-gray-600'>Add-ons:</span>
                <span className='font-medium text-green-600'>
                  +${Object.values(pricingData.additionalServices || {}).reduce((sum, cost) => sum + cost, 0).toLocaleString()}
                </span>
              </div>
            )}
          </div>
          ...
          {/* Generate Button */}
          <button
            className='w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-4 rounded-lg transition-colors shadow-sm mb-2'

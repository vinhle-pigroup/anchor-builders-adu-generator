...
export class AnchorPDFGenerator {
  ...
  private addClientInfo(client: any) {
    ...
    this.doc.setFont('helvetica', 'normal');
    this.doc.text(`${client.address}, ${client.city}, ${client.state} ${client.zipCode}`, leftCol + 25, this.currentY);
  ...
  private addProjectDetails(project: any) {
    ...
    // Row 1
    this.addProjectField('ADU Type:', project.aduType === 'detached' ? 'Detached (1 Story)' : 'Attached ADU', leftCol);
    ...
    this.addProjectField('Appliances:', project.appliancesIncluded ? 'Included' : 'Not Included', middleCol);
  ...
  private addPricingBreakdown(formData: AnchorProposalFormData) {
    ...
    this.doc.rect(this.leftMargin, this.currentY, this.rightMargin - this.leftMargin, tableHeight, 'F');
    this.doc.setDrawColor(226, 232, 240);
    ...
    calculation.lineItems.forEach((item, index) => {
      if (index % 2 === 0) {
        ...
        this.doc.rect(this.leftMargin, this.currentY - 3, this.rightMargin - this.leftMargin, 6, 'F');
      }
    ...
    this.doc.text(`Markup (${(calculation.markupPercentage * 100).toFixed(0)}%):`, this.leftMargin + 3, this.currentY);
    ...
    this.doc.text(`Cost per square foot: $${Math.round(calculation.pricePerSqFt)} | Living area: ${formData.project.squareFootage} sq ft`, this.leftMargin + 3, this.currentY);

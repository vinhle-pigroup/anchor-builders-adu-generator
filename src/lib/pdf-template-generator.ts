...
export class AnchorPDFTemplateGenerator {
  ...
  private async loadTemplate() {
    try {
      // In production, this will be served from the built assets
      const response = await fetch('/src/templates/anchor-proposal-template.html');
  ...
  generateProposal(formData: AnchorProposalFormData): Blob {
    ...
    const milestones = calculateMilestonePayments(calculation.grandTotal);

    // Prepare template variables
    const templateVars = this.prepareTemplateVariables(formData, calculation, milestones);
    ...
    processedHtml = this.processConditionalSections(processedHtml, formData, calculation, milestones);
  ...
  private prepareTemplateVariables(
    ...
    return {
      ...
      // Logos and images (placeholder base64 data)
  ...
  private processConditionalSections(
    ...
    if (formData.project.selectedAddOns.length > 0) {
      const addOnRows = formData.project.selectedAddOns.map(addOnName => {
        const addOn = calculation.lineItems.find((item: any) => item.description.includes(addOnName));
        return addOn ? 
  ...
  private getUtilitySetupText(utilities: any): string {
    ...
    const water = utilities.waterMeter === 'separate' ? 'separate water meter setup' : 'shared water meter setup';
    return `${gas} & ${water}`;
  }

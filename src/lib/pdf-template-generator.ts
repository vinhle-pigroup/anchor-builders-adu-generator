...
import { loadAnchorLogoBase64 } from './getLogoBase64';
...
export class AnchorPDFTemplateGenerator {
  ...
  private async prepareTemplateVariables(
    ...
    return {
      ...
      ANCHOR_LOGO_BASE64: await this.getAnchorLogoBase64(),
  ...
  private async getAnchorLogoBase64(): Promise<string> {
    // Load the actual Anchor Builders logo from public directory
    try {
      const logoBase64 = await loadAnchorLogoBase64();
      console.log('🎨 Logo loaded for PDF (size:', logoBase64.length, 'chars)');
